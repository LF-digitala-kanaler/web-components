...
function Template({ variant, loading, arrow, size, label, subLabel }) {
	return html`
		<lf-button variant="${variant}" size="${size}">
			${label || 'Button label'}
			${subLabel ? html`<span class="d-block text-sm font-weight-normal font-base">${subLabel}</span>`: ''}
		</lf-button>`
}
...
export const PrimaryWithArrowAndIcon = () => html`
	<lf-button variant="primary" arrow>
		<div slot="start" style="width: 24px; height: 24px; background: pink;"></div>
		Button label
	</lf-button>
`
...
export const SecondaryWithArrow = () => html`
	<lf-button variant="secondary" arrow>
		Button label
`
...
export const LoadingState = () => html`
	<lf-button variant="primary" loading @click="${() => console.log('You should not be able to see this message!')}">
		Button label
	</lf-button>
`
